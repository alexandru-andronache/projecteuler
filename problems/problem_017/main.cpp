...
namespace problem_017 {
    int solve(int n) {
        ...
        for (int i = 20; i <= std::min(n, 99); i += 10) {
            ...
            for (int j = 1; j <= std::min(n, 9); ++j) {
                sum += teens[(i - 20) / 10].size() + numbers[j].size();
            }
        }
        ...
        for (int k = 100; k <= std::min(n, 999); k += 100) {
            ...
            for (int i = 1; i <= 9; ++i) {
                sum += numbers[k / 100].size() + hundred.size() + andLiteral.size() + numbers[i].size();
            }
            for (int i = 10; i <= 19; ++i) {
                sum += numbers[k / 100].size() + hundred.size() + andLiteral.size() + numbersTeen[i - 10].size();
            }
            for (int i = 20; i <= 99; i += 10) {
                sum += numbers[k / 100].size() + hundred.size() + andLiteral.size() + teens[(i - 20) / 10].size();
                for (int j = 1; j <= 9; ++j) {
                    sum += numbers[k / 100].size() + hundred.size() + andLiteral.size() + teens[(i - 20) / 10].size() + numbers[j].size();
                }
            }
        }
